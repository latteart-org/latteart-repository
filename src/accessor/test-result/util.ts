...
import { CommandLineScreenshotCompressor } from "./ScreenshotCompressor";

/**
 * Gets an instance of the image compression class. Returns null if it is disabled in the settings.
 * @returns CommandLineScreenshotCompressor or null
 */
export function createCommandLineScreenshotCompressor(
  isEnabled: boolean,
  isDeleteSrcImage: boolean,
  command: string
): null | CommandLineScreenshotCompressor {
  if (!isEnabled) {
    return null;
  }

  return new CommandLineScreenshotCompressor(command, isDeleteSrcImage);
}
